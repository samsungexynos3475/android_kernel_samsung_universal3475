...
volatile unsigned int disk_key_flag = 0;
		...
		for (j = 0; j < (length + rw_size - 1) / rw_size; j++) {
			...
			if (sector_key == DW_MMC_BYPASS_SECTOR_BEGIN) {
				...
				(host->pdata->quirks & DW_MCI_QUIRK_USE_SMU)) { /* disk encryption */
					...
					if (disk_key_flag) {
						int ret;
						ret = exynos_smc(SMC_CMD_FMP, FMP_KEY_SET, 0, 0);
						if (ret) {
...
#else
#if defined(CONFIG_MMC_DW_64_IDMAC) /*64bit*/
			desc->des8 = DW_MMC_BYPASS_SECTOR_BEGIN;
#else /*32bit*/
			desc->des4 = DW_MMC_BYPASS_SECTOR_BEGIN;
#endif
				 ...
				 struct dw_mci_slot *slot)
{
	...
	if (drv_data && drv_data->misc_control) {
		int ret = 0;
		u32 idle_check = NOTIFY_BUSY;

		ret = drv_data->misc_control(host, CTRL_NOTIFY_IDLE,
				&idle_check);
		if(ret)
			dev_dbg(host->dev, "power mode driver busy check faile!\n");
	}
	if (mrq->cmd->data &&
	...
	__acquires(&host->lock)
{
	...
	if (drv_data && drv_data->misc_control) {
		int ret = 0;
		u32 idle_check = NOTIFY_IDLE;

		ret = drv_data->misc_control(host, CTRL_NOTIFY_IDLE,
				&idle_check);
		if(ret)
			dev_dbg(host->dev, "power mode driver idle check faile!\n");
	}
